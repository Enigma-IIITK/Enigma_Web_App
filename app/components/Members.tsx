...
const MemberTable: React.FC = () => {
    ...
    const [newMember, setNewMember] = useState({ email: '', title: 'Club Lead', status: 'current member' });

    const [isEditModalOpen, setIsEditModalOpen] = useState(false);
    const [editMember, setEditMember] = useState({ id: '', email: '', title: '', status: '' });
    ...
    const handleEditFormSubmit = async (e: React.FormEvent) => {
        ...
        const { result, error } = await updateData("members", id, { email, title, status }); // Assuming addData is used for updates too
    ...
    useEffect(() => {
        const fetchMembers = async () => {
            const { result, error } = await getMemberData(); // Adjust this line if you need to pass a specific year
            if (error) {
                setError(error);
                            ...
                            <div className="p-4 md:p-5">
                                <form className="space-y-4" onSubmit={handleFormSubmit}>
                                    <div>
                                        ...
                                        <input type="email" name="email" id="email" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" placeholder="name@iiitkottayam.ac.in" required onChange={handleInputChange} />
                                    </div>
                                    <div>
                                        <label htmlFor="title" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Member Title</label>
                                        <select name="title" id="title" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" required onChange={handleInputChange}>
                                    ...
                                    <div>
                                        ...
                                        <input type="text" name="status" id="status" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" readOnly value="current member" onChange={handleInputChange} />
                                    </div>
                        ...
                        <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
                            ...
                            <div className="p-4 md:p-5">
                                <form className="space-y-4" onSubmit={handleEditFormSubmit}>
                                    ...
                                    <div>
                                        <label htmlFor="status" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Status</label>
                                        <select name="status" id="status" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" value={editMember.status} onChange={handleEditInputChange} required >
